...
export const AnimatedChip = ({
  ...
  const animatedContainerStyle = useAnimatedStyle<
    Required<Partial<Pick<ViewStyle, 'width' | 'paddingHorizontal' | 'backgroundColor'>>>
  >(() => {
    return {
  ...
  const iconStylez = useAnimatedStyle<Partial<Required<Pick<ViewStyle, 'transform'>>>>(() => {
    return {
      transform: [
        {
          translateX: isActive ? interpolate(progress.value, [0, 1], [-8, -5]) : 0,
        },
      ],
    };
  });
    ...
    <Pressable onPress={handlePress}>
      <Animated.View style={[styles.chip, animatedContainerStyle]}>
        ...
        <Animated.View
          style={[
            styles.labelWrapper,
            !isActive && styles.labelWrapperCollapsed,
            animatedTextStyle,
          ]}
