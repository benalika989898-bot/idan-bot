...
> = memo<IProvider>(
  ({
    ...
    const hasSlots: boolean = triggerContent !== null || expandedContent !== null;
    // Keep mounted during collapse animation, hide after animation ends
    const [mounted, setMounted] = useState<boolean>(false);
    const isVisible: boolean = hasSlots && (mounted || triggerContent !== null);
    ...
    const blurIntensity = useAnimatedProps<BlurViewProps>(() => ({
      intensity: interpolate(progress.value, [0, 1], [0, 60]),
    }));
    ...
    return (
      <DynamicIslandContext.Provider value={contextValue}>
        {isVisible && (
          ...
          >
            {/* Backdrop — closes island on tap */}
            <Animated.View
              style={[
                {
                  position: "absolute",
                  top: -(config.topOffset ?? 0),
                  left: -(config.expandedWidth / 2),
                  width: config.expandedWidth + config.expandedWidth,
                  height: screenHeight + (config.topOffset ?? 0),
                  backgroundColor: "#000",
                },
                backdropStyle,
              ]}
